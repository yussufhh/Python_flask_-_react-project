...
const InstructorLogin = () => {
  ...
  const onFinish = async (values) => {
    ...
    try {
      const response = await axios.post('http://localhost:5002/api/auth/instructor/login', {
        email,
        password,
      });
      ...
      if (response.status === 200) {
        ...
        navigate('/adminsidebar'); // Redirect to instructor's dashboard after successful login
      }
    } catch (error) {
        ...
        <Form name="login" onFinish={onFinish} initialValues={getInitialValues()} scrollToFirstError>
          ...
          <Form.Item>
            ...
            <p className="text-center mt-4">
              New instructor? <Link to="/instructor-signup">Create Account</Link>
            </p>
          </Form.Item>
        </Form>
      </div>
